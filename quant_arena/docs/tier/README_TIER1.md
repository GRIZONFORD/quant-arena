...
> `PanelDataManager` → `quant_arena/data/panel_data_manager.py` ·
> `CrossSectionalEngine`/`PortfolioResult` → `quant_arena/backtesting/cross_sectional_engine.py` ·
> `BetaNeutralizer` → `quant_arena/metricas/beta_neutralizer.py`.
> Imports de paquete (p.ej. `from quant_arena.backtesting import CrossSectionalEngine`).
> Los ejemplos con imports planos corresponden al desarrollo original en `legacy/TTT/`.
